import { createSubscribable } from "./utils-DhleAZ_l.js";
import { eventSource, getContext } from "@llamaindex/workflow-core";
...
* @public
*/
var WorkflowStream$1 = class WorkflowStream$1 extends ReadableStream {
	#stream;
	#subscribable;
	/**
	...
	*/
	static fromReadableStream(stream) {
		return new WorkflowStream$1(null, stream.pipeThrough(new TransformStream({ transform: (event, controller) => {
			controller.enqueue(event);
		} })));
	}
	/**
	...
	*/
	static fromResponse(response, eventMap$1) {
		...
		return new WorkflowStream$1(null, body.pipeThrough(new TextDecoderStream()).pipeThrough(new JsonDecodeTransform(eventMap$1)));
	}
	/**
	...
	*/
	...
	pipeThrough(transform, options) {
		...
		return new WorkflowStream$1(null, stream);
	}
	/** @internal */
	...
	tee() {
		...
		return [new WorkflowStream$1(this.#subscribable, l), new WorkflowStream$1(this.#subscribable, r)];
	}
	/**
...
const _executorAsyncLocalStorage = new AsyncContext.Variable();
/**
...
const handlerContextAsyncLocalStorage = new AsyncContext.Variable();
			...
			const createSnapshotFn = (context) => {
				return async function snapshotHandler() {
					...
					const serializable = {
						...
						unrecoverableQueue: queue.filter((event) => !eventCounterWeakMap.has(eventSource(event))).map((event) => event.data),
						version: getVersion(),
