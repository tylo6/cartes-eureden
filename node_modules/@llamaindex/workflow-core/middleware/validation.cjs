...
function withValidation(workflow, validation) {
	const createSafeSendEvent = (...events) => {
		...
		const store = (0, __llamaindex_workflow_core.getContext)();
	...
	return {
		...
		strictHandle: (accept, handler) => {
			const wrappedHandler = (...events) => {
				const context = (0, __llamaindex_workflow_core.getContext)();
		...
		createContext() {
			...
			context.__internal__call_context.subscribe((context$1, next) => {
				(0, __llamaindex_workflow_core.getContext)().safeSendEvent = createSafeSendEvent(...context$1.inputs);
				next(context$1);
			});
			return context;
		}
