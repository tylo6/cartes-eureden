 ...
 */ class FileReader {
    async loadData(filePath) {
        ...
        if (filePath.startsWith("http://") || filePath.startsWith("https://")) {
            ...
            filename = env.path.basename(url.pathname) || "url_document";
        } else {
            // Handle local file
            fileContent = await env.fs.readFile(filePath);
            filename = env.path.basename(filePath);
        }
...
const anyFunctionSchema = v3.z.function(v3.z.tuple([]).rest(v3.z.any()), v3.z.any());
const toolMetadataSchema = v3.z.object({
    description: v3.z.string(),
    name: v3.z.string(),
    parameters: v3.z.record(v3.z.any())
});
const baseToolSchema = v3.z.object({
...
const agentParamsSchema = v3.z.array(baseToolWithCallSchema);
const sentenceSplitterSchema = v3.z.object({
    chunkSize: v3.z.number({
        description: "The token chunk size for each chunk."
    }).gt(0).optional().default(()=>index_cjs.Settings.chunkSize ?? 1024),
    chunkOverlap: v3.z.number({
        description: "The token overlap of each chunk when splitting."
    }).gte(0).optional().default(200),
    separator: v3.z.string({
        description: "Default separator for splitting into words"
    }).default(" "),
    paragraphSeparator: v3.z.string({
        description: "Separator between paragraphs."
    }).optional().default("\n\n\n"),
    secondaryChunkingRegex: v3.z.string({
        description: "Backup regex for splitting into sentences."
    }).optional().default("[^,.;。？！]+[,.;。？！]?"),
    extraAbbreviations: v3.z.array(v3.z.string(), {
...
const sentenceWindowNodeParserSchema = v3.z.object({
    windowSize: v3.z.number({
        description: "The number of sentences on each side of a sentence to capture."
    }).gt(0).default(3),
    windowMetadataKey: v3.z.string({
        description: "The metadata key to store the sentence window under."
    }).default("window"),
    originalTextMetadataKey: v3.z.string({
        description: "The metadata key to store the original sentence in."
    }).default("originalText")
});
const tokenTextSplitterSchema = v3.z.object({
    chunkSize: v3.z.number().positive().default(index_cjs.DEFAULT_CHUNK_SIZE),
    chunkOverlap: v3.z.number().nonnegative().default(index_cjs.DEFAULT_CHUNK_OVERLAP),
    separator: v3.z.string().default(" "),
    backupSeparators: v3.z.array(v3.z.string()).default([
        "\n"
    ])
});
...
exports.agentParamsSchema = agentParamsSchema;
