...
var index_cjs$4 = require('../../schema/dist/index.cjs');
require('magic-bytes.js');
var index_cjs$5 = require('../../vector-store/dist/index.cjs');
 ...
 */ class VectorMemoryBlock extends BaseMemoryBlock {
    constructor(options){
        ...
        this.formatTemplate = options.formatTemplate ?? index_cjs$3.defaultContextSystemPrompt;
    ...
    async get(messages = []) {
        ...
        // Format the results
        const retrievedText = processedNodes.map(({ node })=>node.getContent(index_cjs$4.MetadataMode.NONE)).join("\n\n");
    ...
    async put(messages) {
        ...
        const textNode = new index_cjs$4.TextNode({
            text: texts.join("\n"),
            metadata: {
    ...
    buildDefaultQueryOptions(options) {
        const { similarityTopK = 2, mode = index_cjs$5.VectorStoreQueryMode.DEFAULT, sessionFilterKey = "session_id" } = options ?? {};
...
class Memory {
   ...
   */ this.memoryCursor = 0;
        ...
        this.logger = options.logger ?? env.consoleLogger;
    ...
    initLLM(llm) {
        ...
        try {
            this.llm = llm ?? index_cjs.Settings.llm;
        } catch (error) {
            this.llm = undefined;
        }
    }
...
exports.VectorMemoryBlock = VectorMemoryBlock;
...
exports.vectorBlock = vectorBlock;
